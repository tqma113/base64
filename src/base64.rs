...
pub fn encode(input: &String) -> String {
    ...
    let transform = |bytes: (u8, u8, u8)| -> Vec<u8> {
        vec![
            ENCODE_DICT[(bytes.0 >> 2) as usize] as u8,
            ENCODE_DICT[(((bytes.0 << 4) & MASK) | (bytes.1 >> 4)) as usize],
            ENCODE_DICT[(((bytes.1 << 2) & MASK) | (bytes.2 >> 6)) as usize],
            ENCODE_DICT[(bytes.2 & MASK) as usize],
        ]
    };
    for i in 0..(len / 3) {
        let index = i * 3;
    ...
    match len % 3 {
        1 => {
            result.push(ENCODE_DICT[(bytes[len - 1] >> 2) as usize]);
            result.push(ENCODE_DICT[(bytes[len - 1] << 4 & MASK) as usize]);
...
mod tests {
    ...
    #[test]
    fn decode_test() {
        assert_eq!(super::decode(&"TWFu".to_string()), "Man".to_string());
        assert_eq!(super::decode(&"QQ==".to_string()), "A".to_string());
        assert_eq!(super::decode(&"QkM=".to_string()), "BC".to_string());
    }
}
